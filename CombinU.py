...
## plot damage efficiency versus damage energy
# TdamSi28 = numpy.concatenate([globals()[f'TdamSi28_MT{mts}'] for mts in MTreacSi28])
# TdamSi29 = numpy.concatenate([globals()[f'TdamSi29_MT{mts}'] for mts in MTreacSi29])
# TdamSi30 = numpy.concatenate([globals()[f'TdamSi30_MT{mts}'] for mts in MTreacSi30])
# TdamSi_isotopes = numpy.concatenate([TdamSi28, TdamSi29, TdamSi30])
# TdamSi = numpy.unique(TdamSi_isotopes)
# DamEffSi_SiC = numpy.interp(TdamSi, T_dam_Si_ref, Dam_eff_Si_ref)

# TdamC = numpy.unique(numpy.concatenate([globals()[f'TdamC12_MT{mts}'] for mts in MTreacC12]))
# DamEffC_SiC = numpy.interp(TdamC, T_dam_C_ref, Dam_eff_C_ref)

# ## print the damage efficiencies to file

# ofile = open('DamageEfficiencyData-plot-fit.txt', 'w')
# ## Si in SiC
# print('Silicon in Silicon Carbide', file = ofile)
# print(len(TdamSi), file = ofile)
# for i in range(len(TdamSi)):
	# print(TdamSi[i], DamEffSi_SiC[i], file = ofile)
# ## C in SiC
# print('Carbon in Silicon Carbide', file = ofile)
# print(len(TdamC), file = ofile)
# for i in range(len(TdamC)):
	# print(TdamC[i], DamEffC_SiC[i], file = ofile)

# ## get smaller data sets of damage efficiencies versus damage energy

# TdamC_data = [10, 30, 100, 300, 800, 3000, 8000, 20000, 50000]
# DamEffC_SiC_data = numpy.interp(TdamC_data, TdamC, DamEffC_SiC)
# TdamSi_data = [10, 30, 100, 300, 800, 3000, 8000, 20000, 50000, 150000]
# DamEffSi_SiC_data = numpy.interp(TdamSi_data, TdamSi, DamEffSi_SiC)

# ## print this data sets as well to the file

# print('====================================', file = ofile)
# print('Smaller Data Sets of Si and C in SiC', file = ofile)
# print('Si in SiC', file = ofile)
# for i in range(len(TdamSi_data)):
	# print(TdamSi_data[i], DamEffSi_SiC_data[i], file = ofile)

# print('C in SiC', file = ofile)
# for i in range(len(TdamC_data)):
	# print(TdamC_data[i], DamEffC_SiC_data[i], file = ofile)

# ## read only the data from Devanathan et al. work

# damageenergySi_Devanathan = numpy.zeros(8)
# damageefficiencySi_Devanathan = numpy.zeros(8)
# damageenergyC_Devanathan = numpy.zeros(5)
# damageefficiencyC_Devanathan = numpy.zeros(5)

# ifile = open('Devanathan-et-al-Simulation-Si-on-SiC.txt', 'r')
# ifile.readline()
# ifile.readline()
# for i in range(8):
	# data = ifile.readline().split()
	# damageenergySi_Devanathan[i] = float(data[0])*1000.0
	# damageefficiencySi_Devanathan[i] = float(data[1])
# ifile.close()

# ifile = open('Devanathan-et-al-Simulation-C-on-SiC.txt', 'r')
# ifile.readline()
# ifile.readline()
# for i in range(5):
	# data = ifile.readline().split()
	# damageenergyC_Devanathan[i] = float(data[0])*1000.0
	# damageefficiencyC_Devanathan[i] = float(data[1])
# ifile.close()


# dpival = 150
# fig1 = plt.figure(dpi=dpival)
# plt.xscale('log')
# plt.xlabel('Damage energy (eV)')
# plt.ylabel('Damage efficiency')
# plt.fill_between(TdamSi, DamEffSi_SiC-0.15*DamEffSi_SiC, DamEffSi_SiC+0.15*DamEffSi_SiC, alpha = 0.3, color = 'blue')
# plt.fill_between(TdamC, DamEffC_SiC-0.15*DamEffC_SiC, DamEffC_SiC+0.15*DamEffC_SiC, alpha = 0.3, color = 'red')
# plt.scatter(TdamSi_data, DamEffSi_SiC_data, marker = 'o', color = 'blue')
# plt.plot(TdamSi, DamEffSi_SiC, color = 'blue', label = 'Si in SiC')
# plt.scatter(TdamC_data, DamEffC_SiC_data, marker = 'd', color = 'red')
# plt.plot(TdamC, DamEffC_SiC, color = 'red', label = 'C in SiC')
# plt.scatter(damageenergySi_Devanathan, damageefficiencySi_Devanathan, marker = '*', color = 'black', label = 'Si in 3C-SiC (Devanathan et al.)')
# plt.scatter(damageenergyC_Devanathan, damageefficiencyC_Devanathan, marker = 's', color = 'green', label = 'C in 3C-SiC (Devanathan et al.)')
# plt.legend()
